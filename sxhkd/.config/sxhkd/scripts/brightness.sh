...
if [ "`echo "${brightness} <= 40" | bc`" = 1 ]
then
  text=""
fi
if [ "`echo "${brightness} >= 50" | bc`" = 1 ]
then
  text=""
fi
if [ "`echo "${brightness} >= 60" | bc`" = 1 ]
then
  text=""
fi
if [ "`echo "${brightness} >= 70" | bc`" = 1 ]
then
  text=""
fi
if [ "`echo "${brightness} >= 80" | bc`" = 1 ]
then
  text=""
fi
if [ "`echo "${brightness} >= 90" | bc`" = 1 ]
then
  text=""
fi
if [ "`echo "${brightness} == 100" | bc`" = 1 ]
then
  text=""
fi
if [ "`echo "${brightness} > 100" | bc`" = 1 ]
